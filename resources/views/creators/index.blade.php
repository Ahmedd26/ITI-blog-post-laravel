...
<div class="flex justify-between items-center mb-4">
    <h1 class="text-2xl dark:text-zinc-100 text-zinc-700">Recent Creators</h1>
    <a href="{{route("creators.create")}}" class="btn btn-primary">Create creator</a>
</div>
            ...
            @foreach($creators as $creator)
                <tr
                    ...
                    <td class="px-6 py-4 flex items-center gap-2">
                        <a href="{{route("creators.show", $creator->id)}}" class="btn btn-tertiary">View</a>
                        <a href="{{route("creators.edit", $creator->id)}}" class="btn btn-primary">Edit</a>
                        <button data-modal-target="{{'popup-modal' . $creator->id}}"
                            data-modal-toggle="{{'popup-modal' . $creator->id}}" class="btn btn-danger" type="button">
                            Delete
                        </button>

                        <!-- Confirm Delete Modal -->
                        <div id="{{'popup-modal' . $creator->id}}" tabindex="-1"
                            ...
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-zinc-700">
                                    <button type="button"
                                        ...
                                        data-modal-hide="{{'popup-modal' . $creator->id}}">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-zinc-500 dark:text-zinc-400">Are you sure
                                            you want to delete this creator?</h3>
                                        <div class="flex items-center justify-center gap-2">
                                            <form action="{{route("creators.destroy", $creator->id)}}" method="post">
                                                @csrf
                                                @method("delete")
                                                <button type="submit" data-modal-hide="{{'popup-modal' . $creator->id}}"
                                                    class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                    Yes, I'm sure
                                                </button>
                                            </form>
                                            <button data-modal-hide="{{'popup-modal' . $creator->id}}" type="button"
